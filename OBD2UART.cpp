...
// SoftwareSerial obdSSerial(10, 11); // RX, TX
...
void COBD::readPIDAsync(byte pid)
{
	char cmd[8];
	sprintf(cmd, "%02X%02X\r", dataMode, pid);
	write(cmd);
...
char* COBD::getResponseNoBlock(byte& pid, char* buffer, byte bufsize)
{
	while (recieveIfDataAvailable(buffer, bufsize) > 0) {
...
bool COBD::getResultNoBlock(byte& pid, int& result)
{
	char buffer[64];
	char* data = getResponseNoBlock(pid, buffer, sizeof(buffer));
	if (!data) {
		recover();
		errors++;
		return false;
...
byte COBD::begin()
{
   ...
   if(OBDUART) OBDUART.end();
	...
	for (byte n = 0; n < sizeof(baudrates) / sizeof(baudrates[0]); n++) {
		...
		if (version != 0){
#ifdef DEBUG
			DEBUG.print("Found device at baud "); DEBUG.println(baudrates[n]);
#endif
			break;
		}
		OBDUART.end();		 
...
int COBD::recieveIfDataAvailable(char* buffer, int bufsize, unsigned int timeout)
{
	unsigned char n = 0;
	unsigned long startTime = millis();
	char c = 0;
	while(OBDUART.available()){
		// for (;;) {
			// if (OBDUART.available()) {
				c = OBDUART.read();
				if (!buffer) {
				...
				if(!OBDUART.available()) delay(1); //delayMicroseconds(750);
			// } else {
			// 	if (c == '>') {
...
bool COBD::init(OBD_PROTOCOLS protocol)
{
	...
	for (byte n = 0; n < 2; n++) {
		...
		return true; //@Jonathan added



		stage = 2;
		delay(10);
		sendCommand("010D\r", buffer, sizeof(buffer), OBD_TIMEOUT_LONG);
		delay(1000);
...
void COBD::setBaudRateNoSwitch(unsigned long baudrate)
{
    OBDUART.print("ATBR1 ");
