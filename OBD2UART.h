...
public:
	...
	bool getResultNoBlock(byte& pid, int& result);
...
protected:
	...
	virtual char* getResponseNoBlock(byte& pid, char* buffer, byte bufsize);
	int recieveIfDataAvailable(char* buffer, int bufsize, unsigned int timeout = OBD_TIMEOUT_SHORT);
